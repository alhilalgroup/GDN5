    ...
    jQuery.getJSON('http://cdn.gdnonline.com/homenews?jsoncallback=?', function (data) {
        ...
        jQuery.each(data, function (key, value) {
            ...
            if (categoryname != data[key].category) {
                contenthtml += '<div class="homecategory"    onclick="javascript:window.location.href=\'section.html?secid=' + data[key].category + ' \'"  >' + data[key].category.replace('Local', 'Bahrain').replace('International','World') + '</div>';
                if (screen.width < 768) {
